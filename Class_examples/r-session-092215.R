...
bioassay <-as.data.frame(cbind(
  c(-.86,-.3,-.05,.73),
  c(0,1,3,5),
  c(5,5,5,5)
))
colnames(bioassay) <- c("logdose","deaths","n")
...
ilogit <- function (x) 1 / (1 + exp(-x))# I think logit isn't automatically defined in R
...
ll <- matrix(nrow=m, ncol=m) # log-likelihood grid
for (ia in 1:m)
  for (ib in 1:m)
    ll[ia, ib] <- lhood1(alpha[ia], beta[ib])
pab <- exp(ll - log(sum(exp(ll)))) # posterior on alpha, beta
# plot
persp(alpha, beta, pab)
image(alpha, beta, pab)
points(coef(gb)[1], coef(gb)[2], pch=3)
contour(alpha, beta, pab, add=TRUE)

# sample alpha and beta from the distribution
pa <- rowSums(pab) # marginal on alpha (wrt beta?)
ns <- 1000 # #samples
alpha.s <- beta.s <- numeric(ns)
for (s in 1:ns) {
  ia <- sample.int(m, 1, prob=pa) # sample alpha
  ib <- sample.int(m, 1, prob=pab[ia,] / pa[ia]) # sample beta | alpha
...
lprior <- function(a,b){
  ...
  return(-.5*t(v) %*% ab.prec %*% v)
}

lj <- matrix(nrow=m, ncol=m) # log-JOINT grid
for (ia in 1:m)
  for (ib in 1:m)
    ll[ia, ib] <- lhood1(alpha[ia], beta[ib]) + lprior(alpha[ia], beta[ib]) #before the lprior = 0
pab <- exp(ll - log(sum(exp(ll)))) # posterior on alpha, beta
persp(alpha, beta, pab)
image(alpha, beta, pab)
points(coef(gb)[1], coef(gb)[2], pch=3)
